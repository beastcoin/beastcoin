...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BeastCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BeastCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	double phase = ((double)nHeightM) / 525600.0 * 24.0 * M_PI;

	nSubsidy += ((int)(1700.0 * sin(phase))) * COIN;
...
static const int64 nTargetTimespan = 30 * 60;						// BeastCoin: every 30 minutes
static const int64 nTargetSpacing = 60;								// BeastCoin: 60 sec
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal beastcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BeastCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = " January 23rd, 2014 — Beasts begin to emerge from hidden corners of the world.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // BeastCoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
